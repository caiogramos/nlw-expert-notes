...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvailable) {
      return toast.error('Navegador não suporta a API');
    }
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex-col flex bg-slate-700 p-5 text-left gap-3 outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
          ...
          <form className="flex-1 flex flex-col">
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {shouldShowOnBoarding ? (
                <p className="text-sm leading-6 text-slate-400">
                  Comece <button type="button" onClick={handleStartRecording} className="font-medium text-lime-400 hover:underline">gravando uma nota</button> em áudio ou se preferir <button
